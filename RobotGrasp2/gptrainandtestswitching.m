function [aucave,aucerr,aucstd,ave,uppererr,lowererr,upperstd,lowerstd,kstat,kstatsterr,kstatstd,mfout,indexout,track,seg] = gptrainandtestswitching(x,y,y2,index,groundtruth,loopn,leave,cutoff,segment)
...
%Segment X for switching algorythim
seg = min(x(:,1)):(max(x(:,1))-min(x(:,1)))/segment:max(x(:,1));
seg(1) = [];
track = zeros(loopn,length(seg));
...
%      disp(size(mf))    
    ...
    for i = 1:length(seg)
        if i == 1
            tempind = testx(:,1)<=seg(i);
        else
            tempind = (((testx(:,1)<=seg(i))+0) .* ((testx(:,1)>seg(i-1))+0))>0;
            %Captures values between previous and current step
        end
%         disp(size(tempind))
        mftemp1 = mf1(tempind);
        mftemp2 = mf2(tempind);
