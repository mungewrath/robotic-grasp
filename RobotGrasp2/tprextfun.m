...
%   Created 24OCT13 - RJC
%   Edited 6-20-14 - generalized technique of chopping off initial low FPR
%   values (when they would otherwise cause errors).

finished = 0;
rejectCount = 0;

while ~finished
    if rejectCount >= length(ave)
        tpr = [NaN NaN NaN];
        tprerr = [NaN NaN NaN];
        tprstd = [NaN NaN NaN];
    ...
    try
        tpr = interp1(ave(rejectCount:end,1)',ave(rejectCount:end,2)',fpr');
        tprerr = interp1(ave(rejectCount:end,1)',uppererr(rejectCount:end,2)',fpr');
        tprstd = interp1(ave(rejectCount:end,1)',upperstd(rejectCount:end,2)',fpr');
        finished = 1;
    catch
        rejectCount = rejectCount+1;
        fprintf('Interpolation failed - throwing out %d points\n', rejectCount);
    end
