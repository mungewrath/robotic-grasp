...
datacondition = 4;  %Select which dataset you want
...
%     %dummy vector to get size
%     [~, ~, ~, testx, ~, ~, ~] = leaverand2(x,y,index,leave);
    covfunc = @covSEard;

%     perfx = zeros(length(testx),loopn);
%     perfy = perfx;
%     perft = perfx;
    ...
    for l = 1:loopn
        ...
        if l==1 && m==1
            %Seed Perf matrix on initial pass
            [perfx,~,~,~] = perfcurve(res',mf',1);  
            perfx = zeros(length(perfx),loopn);
            perfy = perfx;
            perft = perfx;
        end
        
        try
            %[perfx,perfy,perft,auc(l,m)] = perfcurve(res',mf',1); %For
            %debugging
