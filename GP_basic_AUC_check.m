...
d_zhifei_autograsp = 1;% <<<TURN ON HERE<<<<<
if d_zhifei_autograsp > 0   
...
if d201301 > 0
    ...
    % Physical testing
    results2 = [datatemp.data(:,dimensionCount+3)];
    header = datatemp.colheaders(2:end);
    
    groundtruth = results2;
    index = (1:length(results2))';
end
...
for l = 1:loopn
    ...
    testIndices = randomIndices(x,length(testx));
    testx = x(testIndices,:);
    disp(testx)
    disp(testIndices)
