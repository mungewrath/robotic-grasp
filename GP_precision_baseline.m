...
%% 5/13/14
%  Trains a GP and outputs predictions based on physical testing. Used for
%  the poster as a baseline for the LR hybrid classifier.
%%
function GP_precision_baseline(energycutoff,binThreshold)

%me = mfilename;
me = sprintf('kd_global_GP_egy_%d_bin_%d',energycutoff,binThreshold);
use_timestamp = false;
thesisStartup;
...
for confidence_cutoff = confidence_thresholds
    ...
    while(l <= loopn)
        ...
        if(train2Proportion >= 1)
            ...
            train2x = zeros(floor(size(data_pca,1) * train2Proportion),1);
        end
        if(testProportion >= 1)
            testx = zeros(testProportion,1);
        else
            testx = zeros(floor(size(data_pca,1) * testProportion),1);
        end
        ...
        [mf_unfiltered, s2f, fmu, fs2] = gp(hyp, inffunc, meanfunc, covfunc, likfunc,trainData,trainy,data_pca(testIndices,:));
