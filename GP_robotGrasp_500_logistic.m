% GP_robotGrasp_500_logistic.m
% Calculates GPs trained on physical testing as well as energy. 
% Update 5/23/14 - fixed dataset, cleaned up setup code, and fixed
% erroneous normalizeer calls.
%%
%//////////////////////////////////////////////////////////////////////////
%------Start LOAD FILES---------------------------------------
%//////////////////////////////////////////////////////////////////////////
...
for l = 1:loopn
    ...
    if(gateTrainProportion >= 1)
        testx = zeros(gateTrainProportion,2);
    else
        testx = zeros(floor(length(x) * gateTrainProportion),2);
    end
    testIndices = zeros(length(testx),1);
    
    % Added in for comparative testing vs. hybrid
    if(testProportion >= 1)
        testx_final = zeros(testProportion,2);
    else
...
%     while ((i <= length(x) * gateTrainProportion && gateTrainProportion < 1) || (i <= gateTrainProportion && gateTrainProportion >= 1))
...
%         end
%     end
    testIndices = randomIndices(data,length(testIndices));
    disp(testx)
    disp(testIndices)
...
%         end
%     end
    testIndices_final = randomIndices(data,length(testIndices_final),testIndices);
    disp(testx_final==testx)
    disp(testIndices_final)
    
    trainx = x;
    logistic_trainx = rawdata(testIndices,:);
    logistic_testx = rawdata(testIndices_final,:);
    ...
    % Train Logistic gating network with first set of test points
    % 1 corresponds to the second classifier being chosen
    gp1_error = abs(mf - res);
    gp2_error = abs(mf_2 - res);
    logistic_y = (gp2_error < gp1_error);
    B = glmfit(logistic_trainx, [logistic_y ones(size(logistic_y))], 'binomial', 'link', 'logit');
    
    
    % Test hybrid classifier
    [mf, s2f, fmu, fs2] = gp(hyp, inffunc, meanfunc, covfunc, likfunc,trainx,trainy,testx_final);
	[mf_2, s2f_2, fmu_2, fs2_2] = gp(hyp2, inffunc, meanfunc, covfunc, likfunc,trainx,trainy2,testx_final);
    
    res = binshakeresults(testIndices_final);
    %mf = normalizeer(mf);
	%mf_2 = normalizeer(mf_2);
	...
	for i = 1:size(mf_final,1)
        if (Logistic(B(1) + logistic_testx(i,:) * B(2:end)) > .5)
            mf_final(i) = mf_2(i);
        end
	end
    ...
    % Generate ROC curve
    try
        [perfx,perfy,perft,auc(l)] = perfcurve(res',normalizeer(mf_final)',1);
...
saveas(h,strcat(savedir,'logistic_500_',date),'bmp');
