...
Removed PCA since we need 11 dimensions
...
for l = 1:loopn
    ...
    testx = zeros(floor(length(x) * testProportion),size(x,2));
    testIndices = zeros(length(testx),1);
	testx_final = zeros(floor(length(x) * testProportion),size(x,2));
    ...
    B = glmfit(testx, [logistic_y ones(size(logistic_y))], 'binomial', 'link', 'logit');
	...
	for i = 1:size(mf_final,1)
        if (Logistic(B(1) + testx_final(i,:) * B(2:end)) > .5)
            mf_final(i) = mf_2(i);
        end
	end
