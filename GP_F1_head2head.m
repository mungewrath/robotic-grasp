...
%  a confidence metric.
%%
function GP_F1_head2head(crowdsourceCutoff,energycutoff,binThreshold)
...
fileName{end+1} = 'crs';
classlabel{end+1} = (results_voting >= crowdsourceCutoff);
...
fileName{end+1} = 'egy';
classlabel{end+1} = (results1 <= energycutoff);
%%
    ...
    %if confidence_cutoff == 0
        ...
        for s = 1:length(classlabel)
            ...
            for i = 1:length(fprCutoffs)
                ...
                for k = 6:6
                    saveTitle = sprintf('%s/isomap_dumps/522_7-14-14/%s_fpr%02d_k%d_conf_%d',savedir,fileName{s},filterLevel*100,k,confidence_cutoff*10);
